...
<div class="slider-wrapper w-100 overflow-hidden no-scrollbars {sliderWrapperClasses.map((c) => c).join(' ')}">
	<div
		class="slider-body d-grid align-items-center flex-column-gap-1"
		style="grid-template-columns: repeat({rolledImages.length}, 25%);"
		...
		{#each rolledImages as image (image.id)}
			<div
				class="slide d-flex align-items-center justify-content-center {slideClasses
					...
					.join(' ')}"
			>
				<img src="{image.path}" alt="slider" class="object-fit-contain w-50" />
			</div>
		{/each}
	</div>
</div>

<script>
	...
	const videoPath = '/assets/videos/',
		...
		customContent = {
			0: {
				lgNum: 20,
				lgPhrase: 'лет',
				smPhrase: 'разрабатываем, производим и поставляем энерогоэффективное оборудование',
			},
			1: {
				lgNum: 250000,
				lgPhrase: '',
	...
	async function makeCounterAlive() {
		...
		for (let i = 0; i < contentElsViewStatus.length; i++) {
			...
			item.inView = item.elem.getBoundingClientRect().bottom < document.documentElement.clientHeight;
			item = item;

			if (!item.inView) continue;
			await item.counter.displayValue(maxValue);
			item = item;
		}
	...
	onMount(() => {
		contentElsViewStatus = contentEls.map((el) =>
			Object.assign(
				...
				{
					elem: el,
					inView: el.getBoundingClientRect().bottom < document.documentElement.clientHeight,
					counter: new Counter(),
				}
			)
		);
		...
		const int = setInterval(() => {
			if (
				contentElsViewStatus.every(
					(item, i) => item.inView && item.counter.startValue >= customContent[i].lgNum
				)
			)
				clearInterval(int);
			makeCounterAlive();
...
<div class="d-flex flex-column flex-row-gap-1 w-100 h-150-vh bg-clr-white-beige">
	{#each srcs as src, index}
		<BgVideo {src} thumbnail="{thumbnails[index]}">
			...
			>
				...
				<small class="clr-white fs-body-lg fw-regular">{customContent[index].smPhrase}</small>
			</div>
		</BgVideo>
	{/each}
</div>
